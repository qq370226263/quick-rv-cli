...
import { store } from '../src/store';`;
    case 'recoil':
      return `import { RecoilRoot } from 'recoil';`;
    case 'zustand':
      return ''; // Zustand 不需要 Provider
    case 'jotai':
...
function getStateManagerWrapper(component, options) {
  const { stateManager } = options; // 需要从options中获取
  switch(stateManager) {
    case 'redux':
    ...
    case 'zustand':
      return component; // Zustand 不需要 Provider
    case 'jotai':
      return `<Provider>
        ${component}
      </Provider>`;
...
describe('Demo Page', () => {
  test('renders demo page', () => {
    render(
      ${getStateManagerWrapper('<Demo />', options)}
    );
